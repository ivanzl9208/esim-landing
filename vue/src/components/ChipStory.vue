  ...
  <div class="chip-reveal-layer" aria-hidden="true">
    <div class="chip-gradient">
      ...
      <div class="story-benefits-copy">
        <p v-for="(text, index) in STORY_BENEFITS" :key="text" :class="`story-benefit story-benefit-${index + 1}`">{{ text }}</p>
      </div>
      <p class="safety-copy"><template v-for="(character, index) in Array.from(SAFETY_COPY)" :key="index"><br v-if="character === '\n'" class="safety-mobile-break" /><span v-else class="safety-character">{{ character }}</span></template></p>
      <ChipMedia ref="media" />
