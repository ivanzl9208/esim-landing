      ...
      <div class="footer-columns">
        ...
        <nav class="footer-navigation" aria-label="Разделы сайта СберМобайл">
          <section v-for="(group, index) in footerGroups" :key="group.title" class="footer-link-group" :class="{ 'is-open': expanded.includes(index) }">
            ...
            <h3 class="footer-mobile-heading"><button type="button" :aria-expanded="expanded.includes(index)" :aria-controls="`${uid}-links-${index}`" @click="toggle(index)">{{ group.title }}<img draggable="false" :src="asset('ending/footer-chevron.svg')" width="16" height="16" alt="" /></button></h3>
        ...
        <div class="footer-utilities">
          ...
          <a class="footer-service" href="https://lk.sbermobile.ru/"><strong>Кабинет абонента</strong><span>Настроить тариф, подключить услуги, проверить баланс</span><img draggable="false" :src="asset('ending/footer-chevron.svg')" width="16" height="16" alt="" /></a>
        </div>
      </div>
