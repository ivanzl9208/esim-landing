...
Удалены `resultCurtain`, `faqReveal`, вторая ширма, reveal/clip-обёртки FAQ, `inert`-гейты перехода, CSS-переменные смещения и дополнительная длина появления FAQ. `heroRevealFrame` остаётся только у исходного Hero. Между checker и FAQ нет GSAP-движения, перекрытия, spacer или паузы.

Предыдущая сцена заканчивается на прежнем конце safety (39,68). Её последний viewport совмещён с входящим checker через layout-margin сцены; сам checker не закреплён и не трансформируется. Трек 37,4–39,68 сохранён как `outro` исключительно для прежнего ухода плавающей CTA. Все предшествующие треки, первая ширма и существующий CTA-parallax сохранены. При изменении только высоты viewport пройденная длина сцены остаётся стабильной; `visualViewport` позиционирует поле над клавиатурой, IntersectionObserver приостанавливает невидимый marquee.
